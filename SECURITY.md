...
- **Session management.** open-passkey provides opt-in HMAC-SHA256 stateless session cookies (`SessionConfig`). If enabled, sessions are valid until expiry — **there is no built-in revocation**. For immediate session invalidation (e.g., password change, compromised account), consumers must layer a blocklist or use a server-side session store. If session support is not enabled, post-authentication token issuance and session handling are entirely the consumer's responsibility.
