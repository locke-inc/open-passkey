...
- **Session management.** open-passkey authenticates principals and emits authentication results; it does not issue cookies or tokens and does not establish, validate, rotate, or revoke sessions. Those controls belong to the caller's session layer. The independent open-bind project is the reference composition for device-bound sessions.
