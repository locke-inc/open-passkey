// Package passkey provides an Echo framework WebAuthn server binding.
//
...
// registration and authentication ceremonies using the Echo framework.
//
// Usage:
//
...
// RegisterRoutes registers all passkey routes on the Echo instance under the given prefix.
// Routes:
//
...
func (p *Passkey) RegisterRoutes(e *echo.Echo, prefix string) {
	g := e.Group(prefix)
...
func (p *Passkey) BeginRegistration(c echo.Context) error {
	var req struct {
	...
	if req.UserID == "" || req.Username == "" {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "userId and username are required"})
	}
	...
	if _, err := rand.Read(prfSalt); err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "failed to generate PRF salt"})
	}
...
func (p *Passkey) FinishRegistration(c echo.Context) error {
	var req struct {
		UserID       string `json:"userId"`
	...
	if err != nil {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "challenge not found or expired"})
	}
	var stored struct {
	...
	if err := json.Unmarshal([]byte(challengeData), &stored); err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "failed to decode challenge data"})
	}
	...
	if err != nil {
		...
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "registration verification failed"})
	}
...
func (p *Passkey) BeginAuthentication(c echo.Context) error {
	var req struct {
		UserID string `json:"userId"`
	}
...
func (p *Passkey) FinishAuthentication(c echo.Context) error {
	var req struct {
		UserID     string `json:"userId"`
		Credential struct {
			ID       string `json:"id"`
	...
	if err != nil {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "invalid credential ID encoding"})
	}

	storedCred, err := p.config.CredentialStore.Get(credIDBytes)
	if err != nil {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "credential not found"})
	}
	...
	if req.Credential.Response.UserHandle != "" {
		...
		if err != nil {
			return c.JSON(http.StatusBadRequest, map[string]string{"error": "invalid userHandle encoding"})
		}
		if string(userHandleBytes) != storedCred.UserID {
			return c.JSON(http.StatusBadRequest, map[string]string{"error": "userHandle does not match credential owner"})
		}
	}
	...
	if err := p.config.CredentialStore.Update(storedCred); err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "failed to update credential"})
	}
